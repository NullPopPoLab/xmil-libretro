...
BYTE joymng_getstat(void) {
	...
	if (s_cJoyFlag == 0)
	{
		...
		switch(input_devices[0]){
			...
			if(input_state_cb(0, RETRO_DEVICE_KEYBOARD, 0, RETROK_JOYPAD_3))cJoyFlag &= ~JOY_BTN3_BIT;
			if(input_state_cb(0, RETRO_DEVICE_KEYBOARD, 0, RETROK_JOYPAD_4))cJoyFlag &= ~JOY_BTN4_BIT;
			break;
		}
	s_cJoyFlag = cJoyFlag;
